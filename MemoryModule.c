...
BuildImportTable(PMEMORYMODULE module)
{
	...
	for (; !IsBadReadPtr(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++) {
		...
		if (module->modules == NULL) {
			tmp = (HCUSTOMMODULE*)HeapAlloc(GetProcessHeap(), HEAP_FREE_CHECKING_ENABLED, (module->numModules + 1) * sizeof(HCUSTOMMODULE));
		}
		else {
			tmp = (HCUSTOMMODULE*)HeapReAlloc(GetProcessHeap(), HEAP_FREE_CHECKING_ENABLED, module->modules, (module->numModules + 1) * sizeof(HCUSTOMMODULE));
		}
		if (tmp == NULL) {
...
FARPROC MemoryGetProcAddress(HMEMORYMODULE mod, LPCSTR name)
{
	...
	if (HIWORD(name) == 0) {
		...
		if (!module->nameExportsTable) {
			...
			struct ExportNameEntry* entry = (struct ExportNameEntry*)HeapAlloc(GetProcessHeap(), HEAP_FREE_CHECKING_ENABLED, exports->NumberOfNames * sizeof(struct ExportNameEntry));
...
void MemoryFreeLibrary(HMEMORYMODULE mod)
{
	...
	HeapFree(GetProcessHeap(), HEAP_FREE_CHECKING_ENABLED, module->nameExportsTable);
	if (module->modules != NULL) {
		...
		if (module->modules != NULL) {
			HeapFree(GetProcessHeap(), HEAP_FREE_CHECKING_ENABLED, module->modules);
		}
	}
